...
async function request<T>(
  ...
  if (res.status === 401 && token) {
    if (!_isRefreshing) {
      _isRefreshing = tryRefreshToken().finally(() => {
        _isRefreshing = null
      })
    }
...
function buildTicketFormData(data: {
  subject?: string
  message: string
...
export const api = {
  auth: {
    ...
    login: (data: { email: string; password: string }) =>
      request<AuthResponse & { require2fa?: boolean; challengeToken?: string }>(
        "/auth/login",
    ...
    passkey: {
      registerOptions: (token: string) =>
        request<any>("/auth/passkey/register/options", {
          method: "POST",
          token,
        }),
      ...
      authVerify: (challengeId: string, credential: any) =>
        request<AuthResponse>("/auth/passkey/authenticate/verify", {
          method: "POST",
      ...
      delete: (token: string, id: string) =>
        request<{ message: string }>(`/auth/passkeys/${id}`, {
          method: "DELETE",
          token,
        }),
    },
    ...
    sessions: {
      ...
      delete: (token: string, id: string) =>
        request<{ message: string }>(`/auth/sessions/${id}`, {
          method: "DELETE",
          token,
        }),
    },
    ...
    oauth: {
      linked: (token: string) =>
        ...
        >("/auth/oauth/linked", { token }),

      unlink: (token: string, provider: string) =>
        request<{ message: string }>(`/auth/oauth/linked/${provider}`, {
          method: "DELETE",
          token,
        }),
    },
  ...
  users: {
    list: (token: string) => request<UserListItem[]>("/users", { token }),
    ...
    deleteMe: (token: string, deletePanelAccount = true) =>
      request<{ message: string }>("/users/me", {
        method: "DELETE",
        token,
        body: { deletePanelAccount },
      }),

    deleteSummary: (token: string, deletePanelAccount = true) =>
      request<{
        deletePanelAccount: boolean
        localOwnedServers: number
        panelOwnedServers: number
        serversToDelete: number
      }>(`/users/me/delete-summary?deletePanelAccount=${deletePanelAccount}`, {
        token,
      }),
    ...
    updateProfile: (token: string, data: { name?: string }) =>
      request<{
        ...
        name: string | null
        avatar: string | null
      }>("/users/me/profile", { method: "PATCH", token, body: data }),
    ...
    panelAccount: (token: string) =>
      request<{
        ...
        username: string | null
        email: string | null
      }>("/users/me/panel-account", { token }),

    resetPanelPassword: (token: string) =>
      request<{ password: string }>("/users/me/panel-account/reset-password", {
        method: "POST",
        token,
      }),
  },
  ...
  tickets: {
    meta: (token: string) => request<TicketMeta>("/tickets/meta", { token }),

    list: (token: string, status?: TicketStatus) =>
      request<TicketSummary[]>(`/tickets${status ? `?status=${status}` : ""}`, {
        token,
      }),

    get: (token: string, id: string) =>
      request<TicketDetail>(`/tickets/${id}`, { token }),
    ...
    create: (
      ...
      data.attachments?.length
        ? requestForm<TicketDetail>("/tickets", buildTicketFormData(data), token)
    ...
    reply: (token: string, id: string, message: string, attachments?: File[]) =>
      attachments?.length
        ? requestForm<TicketDetail>(
            `/tickets/${id}/messages`,
            buildTicketFormData({ message, attachments }),
            token
          )
    ...
    updateStatus: (token: string, id: string, status: UserTicketStatus) =>
  ...
  settings: {
    ...
    getBrand: () => request<BrandConfig>("/settings/brand", {}),
    ...
    testEmail: (token: string, email: string) =>
      request<{ success: boolean; error?: string }>("/settings/test-email", {
        method: "POST",
        token,
        body: { email },
      }),
  ...
  servers: {
    list: (token: string) => request<ServerListResponse>("/servers", { token }),

    create: (token: string, data: CreateServerData) =>
      request<{
        success: boolean
        server: { uuid: string; uuidShort: string; name: string }
      }>("/servers", { method: "POST", token, body: data }),

    modify: (
      token: string,
      uuid: string,
      data: { ram: number; disk: number; cpu: number }
    ) =>
      request<{ success: boolean }>(`/servers/${uuid}`, {
        method: "PATCH",
        token,
        body: data,
      }),
    ...
    stats: (token: string) => request<PanelStats>("/servers/stats", { token }),

    eggs: (token: string) => request<EggConfig[]>("/servers/eggs", { token }),
    ...
    power: (
      ...
      request<any>(`/servers/${uuid}/power`, {
        method: "POST",
        token,
        body: { action },
      }),
    ...
    command: (token: string, uuid: string, command: string) =>
      request<any>(`/servers/${uuid}/command`, {
        method: "POST",
        token,
        body: { command },
      }),
    ...
    websocket: (token: string, uuid: string) =>
      request<{ token: string; url: string }>(`/servers/${uuid}/websocket`, {
        token,
      }),
    ...
    files: {
      list: (token: string, uuid: string, directory = "/") =>
        request<any>(
          `/servers/${uuid}/files/list?directory=${encodeURIComponent(directory)}`,
          { token }
        ),

      contents: (token: string, uuid: string, file: string) =>
        request<any>(
          `/servers/${uuid}/files/contents?file=${encodeURIComponent(file)}`,
          { token }
        ),

      write: (token: string, uuid: string, file: string, content: string) =>
        request<any>(`/servers/${uuid}/files/write`, {
          method: "POST",
          token,
          body: { file, content },
        }),
      ...
      createDirectory: (
        ...
        request<any>(`/servers/${uuid}/files/create-directory`, {
          method: "POST",
          token,
          body: { root, name },
        }),
      ...
      deleteFiles: (
        ...
        request<any>(`/servers/${uuid}/files/delete`, {
          method: "POST",
          token,
          body: { root, files },
        }),
      ...
      rename: (
        ...
        request<any>(`/servers/${uuid}/files/rename`, {
          method: "PUT",
          token,
          body: { root, files },
        }),
      ...
      compress: (token: string, uuid: string, root: string, files: string[]) =>
        request<any>(`/servers/${uuid}/files/compress`, {
          method: "POST",
          token,
          body: { root, files },
        }),
      ...
      decompress: (token: string, uuid: string, root: string, file: string) =>
        request<any>(`/servers/${uuid}/files/decompress`, {
          method: "POST",
          token,
          body: { root, file },
        }),
      ...
      downloadUrl: (token: string, uuid: string, file: string) =>
        request<any>(
          `/servers/${uuid}/files/download?file=${encodeURIComponent(file)}`,
          { token }
        ),
    ...
    backups: {
      ...
      create: (token: string, uuid: string, name?: string) =>
        request<any>(`/servers/${uuid}/backups`, {
          method: "POST",
          token,
          body: { name },
        }),
      ...
      remove: (token: string, uuid: string, backupUuid: string) =>
        request<any>(`/servers/${uuid}/backups/${backupUuid}`, {
          method: "DELETE",
          token,
        }),

      download: (token: string, uuid: string, backupUuid: string) =>
        request<any>(`/servers/${uuid}/backups/${backupUuid}/download`, {
          token,
        }),

      restore: (token: string, uuid: string, backupUuid: string) =>
        request<any>(`/servers/${uuid}/backups/${backupUuid}/restore`, {
          method: "POST",
          token,
        }),
    },
    ...
    startup: {
      ...
      update: (
        ...
        request<any>(`/servers/${uuid}/startup`, {
          method: "PUT",
          token,
          body: variables,
        }),
      ...
      updateDockerImage: (token: string, uuid: string, image: string) =>
        request<any>(`/servers/${uuid}/docker-image`, {
          method: "PUT",
          token,
          body: { image },
        }),
    ...
    rename: (token: string, uuid: string, name: string) =>
      request<any>(`/servers/${uuid}/rename`, {
        method: "POST",
        token,
        body: { name },
      }),
    ...
    reinstall: (token: string, uuid: string, truncateDirectory = true) =>
      request<any>(`/servers/${uuid}/reinstall`, {
        method: "POST",
        token,
    ...
    plugins: {
      install: (
        ...
        request<{ success: boolean; file: string }>(
          `/servers/${uuid}/plugins/install`,
          { method: "POST", token, body: { downloadUrl, filename } }
        ),
    },
  ...
  store: {
    items: (token: string) => request<StoreItem[]>("/store/items", { token }),
    ...
    buy: (token: string, resource: string, amount: number) =>
      request<{
        ...
        resource: string
        amount: number
      }>("/store/buy", { method: "POST", token, body: { resource, amount } }),

    claimDaily: (token: string) =>
      request<{ success: boolean; amount: number }>("/store/daily", {
        method: "POST",
        token,
      }),

    dailyStatus: (token: string) =>
      request<{ available: boolean; hoursLeft?: number; reason?: string }>(
        "/store/daily",
        { token }
      ),
  },
  ...
  admin: {
    // Eggs
    listEggs: (token: string) => request<EggConfig[]>("/admin/eggs", { token }),
    ...
    updateEgg: (token: string, id: string, data: any) =>
      request<EggConfig>(`/admin/eggs/${id}`, {
        method: "PATCH",
        token,
        body: data,
      }),
    deleteEgg: (token: string, id: string) =>
      request<{ message: string }>(`/admin/eggs/${id}`, {
        method: "DELETE",
        token,
      }),
    ...
    createLocation: (token: string, data: any) =>
      request<LocationConfig>("/admin/locations", {
        method: "POST",
        token,
        body: data,
      }),
    updateLocation: (token: string, id: string, data: any) =>
      request<LocationConfig>(`/admin/locations/${id}`, {
        method: "PATCH",
        token,
        body: data,
      }),
    deleteLocation: (token: string, id: string) =>
      request<{ message: string }>(`/admin/locations/${id}`, {
        method: "DELETE",
        token,
      }),
    ...
    createPackage: (token: string, data: any) =>
      request<PackageConfig>("/admin/packages", {
        method: "POST",
        token,
        body: data,
      }),
    updatePackage: (token: string, id: string, data: any) =>
      request<PackageConfig>(`/admin/packages/${id}`, {
        method: "PATCH",
        token,
        body: data,
      }),
    ...
    upsertStoreItem: (token: string, data: any) =>
      request<StoreItem>("/admin/store-items", {
        method: "POST",
        token,
        body: data,
      }),
    deleteStoreItem: (token: string, id: string) =>
      request<{ message: string }>(`/admin/store-items/${id}`, {
        method: "DELETE",
        token,
      }),
    ...
    getUserResources: (token: string, userId: string) =>
      request<UserResourcesResponse>(`/admin/user-resources/${userId}`, {
        token,
      }),
    ...
    syncLocations: (token: string) =>
      request<SyncPartialResult>("/admin/sync/locations", {
        method: "POST",
        token,
      }),
    syncNodes: (token: string) =>
      request<SyncPartialResult>("/admin/sync/nodes", {
        method: "POST",
        token,
      }),
    ...
    syncServers: (token: string) =>
      request<MigrateServersResult>("/admin/sync/servers", {
        method: "POST",
        token,
      }),
    ...
    deleteRole: (token: string, id: string) =>
      request<{ message: string }>(`/admin/roles/${id}`, {
        method: "DELETE",
        token,
      }),
    ...
    forceVerifyEmail: (token: string, id: string) =>
      request<{ message: string }>(`/admin/users/${id}/verify-email`, {
        method: "POST",
        token,
      }),
    setUserPanelId: (token: string, id: string, calagopusId: string) =>
      request<any>(`/admin/users/${id}/panel-id`, {
        method: "PATCH",
        token,
        body: { calagopusId },
      }),
    ...
    suspendServer: (token: string, uuid: string) =>
      request<{ message: string }>(`/admin/servers/${uuid}/suspend`, {
        method: "POST",
        token,
      }),
    unsuspendServer: (token: string, uuid: string) =>
      request<{ message: string }>(`/admin/servers/${uuid}/unsuspend`, {
        method: "POST",
        token,
      }),
    deleteServer: (token: string, uuid: string) =>
      request<{ message: string }>(`/admin/servers/${uuid}`, {
        method: "DELETE",
        token,
      }),
    ...
    updateServer: (token: string, uuid: string, data: any) =>
      request<any>(`/admin/servers/${uuid}`, {
        method: "PATCH",
        token,
        body: data,
      }),
    listNodeAllocations: (
      ...
      request<any[]>(
        `/admin/servers/${serverUuid}/allocations?nodeUuid=${nodeUuid}`,
        { token }
      ),

    // Tickets
    listTickets: (token: string, status?: TicketStatus) =>
      request<TicketSummary[]>(
        `/admin/tickets${status ? `?status=${status}` : ""}`,
        { token }
      ),
    getTicket: (token: string, id: string) =>
      request<TicketDetail>(`/admin/tickets/${id}`, { token }),
    replyToTicket: (
      token: string,
      id: string,
      message: string,
      ...
      attachments?.length
        ...
        : request<TicketDetail>(`/admin/tickets/${id}/messages`, {
            method: "POST",
            token,
            body: { message },
          }),
    updateTicketStatus: (token: string, id: string, status: TicketStatus) =>
      request<TicketDetail>(`/admin/tickets/${id}/status`, {
        method: "PATCH",
        token,
        body: { status },
      }),
...
export type UserTicketStatus = "OPEN" | "CLOSED"
